...
setwd(file.path('C:', 'Users', 'bpickens','Desktop', 'Seabird_workflow', 'demo'))##

# Input: file name
input1 <- "20241107_152400.aflight"
...
a1$unique_image_jpg <- paste0(a1$unique_image,".jpg")
a1$unique_image_jpg

merge2 <- merge(a1, a5, by="unique_image_jpg")

# Map the detections
# get rid of na's first
a10 <- merge2[complete.cases(merge2), ]

names(a10)

# Projections, crs = 4326--WGS84
upper_left <- st_as_sf(merge2, coords = c("TLLong", "TLLat"), crs= 4326)
plot(upper_left)

# crs for MA, ME= UTMN19/EPSG:32618
# for mid_Atlantic = UTM18, EPSG:32619
utm_upper_left <- st_transform(upper_left, crs = crs1)

utm_upper_left %>% 
  st_coordinates()

names(utm_upper_left)

utm_upper_left2 <- separate_wider_delim (utm_upper_left, cols= "geometry", delim=" ", names= c("ULLong_utm","ULLat_utm"), too_many="drop")

export_spatial <- substr(input1,1,nchar(input1)-8)

utm_upper_left2$ULLat_utm <- substr(utm_upper_left2$ULLat_utm, 1, nchar(utm_upper_left2$ULLat_utm)-1)

utm_upper_left2$ULLong_utm <- substr(utm_upper_left2$ULLong_utm, 3, nchar(utm_upper_left2$ULLat_utm))

names(utm_upper_left2)
